...
func RelayTextHelper(c *gin.Context) *model.ErrorWithStatusCode {
	...
	// get request body
	requestBody, bodyContent, err := getRequestBody(c, meta, textRequest, adaptor)
	if err != nil {
...
func getRequestBody(c *gin.Context, meta *meta.Meta, textRequest *model.GeneralOpenAIRequest, adaptor adaptor.Adaptor) (io.Reader, string, error) {
	if !config.EnforceIncludeUsage &&
		...
		return bytes.NewBuffer(bodyBytes), string(bodyBytes), nil
	}
	...
	return bytes.NewBuffer(jsonData), string(jsonData), nil
}
